...
- **屏幕底部滚轮:** 鼠标在屏幕底部边缘（5px 范围内）时，滚轮调节系统音量
  - 向上滚动增大音量，向下滚动减小音量（每次 ±2）
  - 无需按 CapsLock，裸滚轮即可触发
  - 跟随 CapsLock++ 全局启用/禁用（CapsLock+Esc）
...
- **CapsLock+数字键(1-0):** 显示对应的快捷菜单组
  ...
  - 菜单采用现代化UI设计：圆角窗口、阴影、淡入淡出动画、Win11 原生圆角按钮
...
- **CapsLock+`` ` ``:** 打开帮助面板（热键速查表）
  - 展示所有热键及其功能，按分类组织：
    - 基本功能、光标移动、文本选择、删除操作、编辑操作
    - 窗口管理、鼠标模式、快捷菜单、实用工具
  - 再次按 `CapsLock+` ` `` 或 Esc 或点击外部关闭
  - 面板采用与快捷菜单一致的现代化 UI（圆角、阴影、淡入淡出动画）
